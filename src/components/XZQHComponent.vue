  ...
  methods: {
    ...
    async getProvinceData() {
      ...
      const queryTask = new QueryTask({
        url: 'http://localhost:6080/arcgis/rest/services/MyMapService/FeatureServer/0',
      });
      let query = new Query();
    ...
    async getCityAndCountyData(value) {
      ...
      const [QueryTask, Query] = await loadModules(['esri/tasks/QueryTask', 'esri/tasks/support/Query'], options);
      const queryTask = new QueryTask({
        url: 'http://localhost:6080/arcgis/rest/services/MyMapService/FeatureServer/2',
      });
      let query = new Query();
      query.returnGeometry = false;
      query.outFields = ['*'];
        ...
        Promise.all(
          currentCityData.map(async (item2) => {
            ...
            const queryTask2 = new QueryTask({
              url: 'http://localhost:6080/arcgis/rest/services/MyMapService/FeatureServer/1',
            });
    ...
    async handleItemClick(val, type) {
      ...
      if (type === 'city') {
        ...
        serverUrl = 'http://localhost:6080/arcgis/rest/services/MyMapService/FeatureServer/2';
      } else if (type === 'county') {
        code = val.toString().substring(0, 6);
        // console.log("handleItemClick ",code);
        serverUrl = 'http://localhost:6080/arcgis/rest/services/MyMapService/FeatureServer/1';
      }
      ...
      // console.log(rings);
      const [Polygon, Point] = await loadModules(['esri/geometry/Polygon', 'esri/geometry/Point'], options);
